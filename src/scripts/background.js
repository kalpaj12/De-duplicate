...
function removeSingleDuplicate(duplicatesArr, changefocus) {
  if (changefocus) switchFocus(duplicatesArr[0]);
  else closeDuplicateTab(duplicatesArr[0].id);
}

function removeMultipleDuplicates(duplicatesArr) {
  duplicatesArr.forEach((tab, pos) => {
    if (pos !== duplicatesArr.length - 1) closeDuplicateTab(tab.id);
    else removeSingleDuplicate([tab], true);
  });
}
          ...
          if (tabInfo.url) {
            if (!nonduplicateURLSet.has(tabInfo.url)) {
              ...
              const nonduplicatetabversion = nonduplicateArr.find(
                ({ url, id }) => url === tabInfo.url && id !== tabInfo.id
              );
...
chrome.tabs.onRemoved.addListener(function(tabId, removeInfo) {
  ...
  var tab = nonduplicateArr.find(({ id }) => id == tabId);
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
  ...
  if (changeInfo.discarded && changeInfo.discarded === true) {
    var nonduplicateTab = nonduplicateArr.find(({ url }) => url == tab.url);
    ...
    var updatednewTab = newTabsArr.find(({ id }) => id == tabId);
    ...
    if (updatednewTab) {
      ...
      var duplicateExists = nonduplicateArr.find(
        ({ url, id }) => url === updatednewTab.url && id !== tab.id
      );
